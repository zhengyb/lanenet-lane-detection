...
def list_trt_engine_layers(engine_path: str):
    ...
    # 收集引擎信息
    engine_info = {"inputs": [], "outputs": [], "layers": []}
    ...
    for i in range(engine.num_layers):
        layer_info = inspector.get_layer_information(
...
class TRTLaneNet:
    ...
    def inference(self, image):
        ...
        cuda.memcpy_htod_async(
            self.inputs[0]["allocation"], input_data.ravel(), self.stream
        )
        ...
        for output in self.outputs:
            ...
            cuda.memcpy_dtoh_async(
                outputs[output["name"]], output["allocation"], self.stream
            )
