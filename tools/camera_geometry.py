...
class CameraGeometry(object):
    def __init__(self, camera_name, height=1.3, yaw_deg=0, pitch_deg=-5, roll_deg=0, image_width=1920, image_height=1080,
        ...
        if self.camera_name == CameraName.CARLA or self.camera_name == CameraName.BEV:
            self.intrinsic_matrix = get_intrinsic_matrix4carla(self.image_width, self.image_height, self.field_of_view_deg)
        else:
    ...
    def roadframe_to_camframe(self,vec_in_road_frame):
        return self.rotation_cam_to_road.T @ (vec_in_road_frame - self.translation_cam_to_road)
